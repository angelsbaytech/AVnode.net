  ...
  async.parallel({
    image: (cb) => {
      if (req.files['image']) {
        ...
        if (file.size < config.maxImageSize && file.originalname.match(/\.(jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF)$/)) {
          ...
          if (file.size > config.maxImageSize) {
            logger.debug(`${apiCall} file too large`);
            req.flash('errors', { msg: `${apiCall} file too large` });
            return cb(null);
          }
          // accept image only
          if (!file.originalname.match(/\.(jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF)$/)) {
            req.flash('errors', { msg: `${apiCall} Only image files are allowed` });
            logger.debug(`${apiCall} Only image files are allowed ${file.originalname}`);
            return cb(null);
          }
        }
...
router.post('/user/:id/image/profile', up, (req, res, next) => {
  // FIXME: Why next() as error handling?
  // FIXME: Delete old asset if there is one
  const apiCall = 'api, image profile: ';
  /**
   * file: {"fieldname":"image",
   * "originalname":"hhinvit (2).JPG",
...
router.post('/user/:id/image/teaser', up, (req, res, next) => {
  ...
  async.parallel({
    image: (cb) => {
      if (req.files['image']) {
        ...
        if (file.size < config.maxImageSize && file.originalname.match(/\.(jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF)$/)) {
          ...
          if (!file.originalname.match(/\.(jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF)$/)) {
            req.flash('errors', { msg: `${apiCall} Only image files are allowed` });
            logger.debug(`${apiCall} Only image files are allowed`);
            return cb(null);
          }
        }
      } else {
    ...
    req.flash('success', { msg: `${apiCall} results: ${results}` });
    logger.info(`${apiCall} results: ${JSON.stringify(results)}`);
  ...
  User.findById(req.params.id, (err, user) => {
    ...
    if (user) {
      ...
      user.save((uErr) => {
        ...
        // BL FIXME shown only on page refresh... req.flash('success', { msg: i18n.__('Saved') });
